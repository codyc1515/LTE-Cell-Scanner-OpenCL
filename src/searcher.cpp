...
lte_opencl_t::lte_opencl_t(const uint &platform_id, const uint &device_id): platform_id(platform_id), device_id(device_id) {
	context = 0;
	cmdQueue = 0;
...
int lte_opencl_t::setup_filter_mchn(std::string filter_mchn_kernels_filename, const size_t &capbuf_length_in,
									const size_t &num_filter_in, const size_t &filter_length_in,
									const uint &xcorr_workitem_in) {
	...
	if (!kernel_file.is_open()) {
		cout << "setup_filter_mchn: open file failed! Please make sure program can find "
			 << filter_mchn_kernels_filename << "\n";
		ABORT(-1);
	}
	...
	// ---------------------------------gen buffers---------------------------
	filter_mchn_coef = clCreateBuffer(context, CL_MEM_READ_ONLY, 2 * sizeof(float) * filter_mchn_buf_coef_len, NULL,
									  &ret);
	if (ret != 0) {
	...
	filter_mchn_orig = clCreateBuffer(context, CL_MEM_READ_ONLY, 2 * sizeof(float) * filter_mchn_buf_in_len, NULL,
									  &ret);
	if (ret != 0) {
	...
	filter_mchn_out_abs2 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * filter_mchn_buf_out_len, NULL,
										  &ret);
	if (ret != 0) {
...
//  Real_Timer tt;
//  tt.tic();
	ret = clEnqueueWriteBuffer(cmdQueue, filter_mchn_orig, CL_FALSE, 0, 2 * filter_mchn_buf_in_len * sizeof(float),
							   filter_mchn_in_host, 0, NULL, &(write_done[0]));
//  clFinish(cmdQueue);
...
//  tt.tic();
	ret = clEnqueueWriteBuffer(cmdQueue, filter_mchn_coef, CL_FALSE, 0, 2 * filter_mchn_buf_coef_len * sizeof(float),
							   filter_mchn_coef_host, 0, NULL, &(write_done[1]));
//  clFinish(cmdQueue);
...
//  cout << filter_mchn_num_chn << "\n";
	global_work_size[0] = filter_mchn_num_chn;
...
//  tt.tic();
	ret = clEnqueueNDRangeKernel(cmdQueue, filter_mchn_multi_filter, 1, NULL, global_work_size, local_work_size, 2,
								 write_done, &multi_filter_done);
//  clFinish(cmdQueue);
...
//  tt.tic();
	ret = clEnqueueReadBuffer(cmdQueue, filter_mchn_out_abs2, CL_FALSE, 0, filter_mchn_buf_out_len * sizeof(float),
							  filter_mchn_out_abs2_host, 1, &multi_filter_done, NULL);
//  clFinish(cmdQueue);
...
int lte_opencl_t::setup_filter_my(std::string filter_my_kernels_filename, const size_t &capbuf_length_in,
								  const uint &filter_workitem_in) {
	// in case setup multiple times----------------------------------------
	if (filter_my_in_host != 0) {
		delete[] filter_my_in_host;
		filter_my_in_host = 0;
	...
	if ((filter_my_buf_in_len % filter_my_workitem) != 0) {
		filter_my_buf_in_len =
				filter_my_buf_in_len + (filter_my_workitem - (filter_my_buf_in_len % filter_my_workitem));
	}
...
int lte_opencl_t::filter_my(cvec &capbuf) {
	...
	cl_event write_done;
	ret = clEnqueueWriteBuffer(cmdQueue, filter_my_orig, CL_FALSE, 0, 2 * filter_my_buf_in_len * sizeof(float),
							   filter_my_in_host, 0, NULL, &write_done);
	if (ret != 0) {
	...
	ret = clEnqueueNDRangeKernel(cmdQueue, filter_my_skip2cols, 1, NULL, global_work_size, local_work_size, 1,
								 &write_done, &skip2cols_done);
	if (ret != 0) {
	...
	local_work_size[0] = 1;
	cl_event multi_filter_done;
	ret = clEnqueueNDRangeKernel(cmdQueue, filter_my_multi_filter, 1, NULL, global_work_size, local_work_size, 1,
								 &skip2cols_done, &multi_filter_done);
	if (ret != 0) {
	...
	local_work_size[0] = 1;
	cl_event result_combine_done;
	ret = clEnqueueNDRangeKernel(cmdQueue, filter_my_result_combine, 1, NULL, global_work_size, local_work_size, 1,
								 &multi_filter_done, &result_combine_done);
	if (ret != 0) {
		cout << "clEnqueueNDRangeKernel filter_my_result_combine " << ret << "\n";
		ABORT(-1);
	}

	ret = clEnqueueReadBuffer(cmdQueue, filter_my_out, CL_FALSE, 0, 2 * filter_my_buf_out_len * sizeof(float),
							  filter_my_out_host, 1, &result_combine_done, NULL);
	if (ret != 0) {
	...
	for (uint i = 0; i < filter_my_capbuf_length; i++) {
		complex<float> tmp_val(filter_my_out_host[2 * (i + (filter_my_length - 1) / 2) + 0],
							   filter_my_out_host[2 * (i + (filter_my_length - 1) / 2) + 1]);
		capbuf(i) = tmp_val;
	}
...
int lte_opencl_t::setup_opencl() {
	...
	for (uint pidx = 0; pidx < num_platform; pidx++) {
		...
		if (pidx == platform_id) {
			...
			for (uint didx = 0; didx < numDevices; didx++) {
				...
				if (didx == device_id) {
					...
					cmdQueue = clCreateCommandQueueWithProperties(context, tmp_devices[didx], properties, &ret);
					if (ret != 0) {
		...
		for (uint didx = 0; didx < numDevices; didx++) {
			...
			if (pidx == platform_id && didx == device_id)
			...
			ret = clGetDeviceInfo(tmp_devices[didx], CL_DEVICE_LOCAL_MEM_TYPE, sizeof(cl_device_local_mem_type),
								  &mem_type, NULL);
			if (ret != 0) {
			...
			if (pidx == platform_id && didx == device_id)
				cout << "Platform " << pidx << " Device " << didx << " MAX_COMPUTE_UNITS: " << max_num_cu << "\n";
			...
			DBG(cout << "Platform " << pidx << " Device " << didx << " MAX_WORK_ITEM_SIZES: " << wi_size[0] << " "
					 << wi_size[1] << " " << wi_size[2] << "\n";)

			cl_uint float_vec_size;
			ret = clGetDeviceInfo(tmp_devices[didx], CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT, sizeof(cl_uint),
								  &float_vec_size, NULL);
			if (ret != 0) {
			...
			if (pidx == platform_id && didx == device_id)
				cout << "Platform " << pidx << " Device " << didx << " PREFERRED_VECTOR_WIDTH_FLOAT: " << float_vec_size
					 << "\n";
...
#else
	xc = vector<vector<vector<complex<float> > > >(3, vector<vector<complex<float> > >(len - (len_pss - 1),
...
#else
	xc = vector<vector<vector<complex<float> > > >(3, vector<vector<complex<float> > >(n_cap - 136,
																					   vector<complex<float> >(n_f)));
#endif
...
void sp_est(
	...
	for (uint32 t = 1; t < n_sp; t++) {
		sp[t] = sp[t - 1] +
				(-pow(capbuf[t - 1].real(), 2) - pow(capbuf[t - 1].imag(), 2) + pow(capbuf[t + 274 - 1].real(), 2) +
				 pow(capbuf[t + 274 - 1].imag(), 2)) / 274;
	}
...
//  cout << "PSS xcorr cost " << tt.get_time() << "s\n";
	...
	cout << "\ninput level: avg abs(real) " << (sum(abs(real(s))) / len) << " avg abs(imag) "
		 << (sum(abs(imag(s))) / len) << "\n";
...
//  vec tmp_store(len_short);
	for (uint16 j = 0; j < num_fo_pss; j++) {
		...
		tmp_avg_val = (sum(corr_store_tmp_col) - tmp_peak) /
					  ((double) pss_period - (2.0 * (double) num_half_radioframe + 1.0));
...
//  }
	...
	for (uint16 i = 0; i < len_sort_idx; i++) {
		...
		for (uint32 j = tmp_max_idx; j < len_short; j = j + pss_period) {
			if ((j + 3 + num_half_radioframe) < len_short) {
				...
				if (tmp_idx != 0 && tmp_idx != 2 * (3 + (int) num_half_radioframe)) {
					...
					peak_location =
							((peak_location - 1) * peak_left / sum_peak) + (peak_location * tmp_val / sum_peak) +
							((peak_location + 1) * peak_right / sum_peak);
				} else {
					peak_val[peak_count] = 0;
					DBG(cout << "Seems not a peak "
							 << corr_seq(j - 3 - num_half_radioframe, j + 3 + num_half_radioframe) << " at i=" << i
							 << " j=" << j << "\n";)
				}
		...
		if ((double) (last_idx - first_idx) < (double) num_peak * 1.0 / 2.0) {
			DBG(cout << "Too few peak (actual " << (last_idx - first_idx) << ", need "
					 << ((double) num_peak * 1.0 / 2.0) << ") at i=" << i << " of total " << len_sort_idx << "\n";);
			continue;
		} else {
		...
		for (uint16 j = 0; j < real_count; j++) {
			if (abs(f_tmp - f_set[j]) < 7500 && abs(ppm_tmp - ppm[j]) < 6) {
				exist_flag = true;
				DBG(cout << "duplicated fo and ppm " << (f_tmp / 1.0e3) << "kHz " << ppm_tmp << "PPM at i=" << i
						 << " j=" << j << "\n";)
				break;
			}
		}
...
//  xc_correlate_new(capbuf,f_search_set,pss_fo_set,xc);
	...
	xc_combine(capbuf, xc, fc_requested, fc_programmed, fs_programmed, f_search_set, xc_incoherent_single, n_comb_xc,
			   sampling_carrier_twist, k_factor);
...
//    cout << "peak_search " << peak_pow << " " << peak_ind << " " << Z_th1(peak_ind) << "\n";
		...
		for (uint16 t = peak_ind - ds_comb_arm; t <= peak_ind + ds_comb_arm; t++) {
			...
			if (xc_incoherent_single[peak_n_id_2](xc_incoherent_collapsed_frq(peak_n_id_2, peak_ind), t_wrap) >
				best_pow) {
				best_pow = xc_incoherent_single[peak_n_id_2](xc_incoherent_collapsed_frq(peak_n_id_2, peak_ind),
															 t_wrap);
				best_ind = t_wrap;
			}
		}
...
//      k_factor=(fc_requested-peak_freq)/fc_programmed;
  ...
  sss_ext_raw=NAN;
	...
	for (uint16 k = 0; k < n_pss; k++) {
		...
		h_raw.set_row(k, elem_mult(extract_psss(capbuf.mid(pss_dft_location, 128), -peak_freq, k_factor, fs_programmed),
								   conj(ROM_TABLES.pss_fd[n_id_2_est])));
  ...
  sss_h2_ext_est=NAN;
	...
	for (uint8 t = 0; t < 62; t++) {
		...
		sss_h1_nrm_est(t) = sss_h1_np_est(t) * sum(elem_mult(conj(h_sm_h1), to_cvec(pss_np_inv_h1),
															 sss_nrm_raw.get_col(t).get(
																	 itpp_ext::matlab_range(0, 2, n_pss - 1))));
		sss_h2_nrm_est(t) = sss_h2_np_est(t) * sum(elem_mult(conj(h_sm_h2), to_cvec(pss_np_inv_h2),
															 sss_nrm_raw.get_col(t).get(
																	 itpp_ext::matlab_range(1, 2, n_pss - 1))));
		// Estimate SSS assuming extended CP
		sss_h1_ext_est(t) = sss_h1_np_est(t) * sum(elem_mult(conj(h_sm_h1), to_cvec(pss_np_inv_h1),
															 sss_ext_raw.get_col(t).get(
																	 itpp_ext::matlab_range(0, 2, n_pss - 1))));
		sss_h2_ext_est(t) = sss_h2_np_est(t) * sum(elem_mult(conj(h_sm_h2), to_cvec(pss_np_inv_h2),
															 sss_ext_raw.get_col(t).get(
																	 itpp_ext::matlab_range(1, 2, n_pss - 1))));
	}
}
...
double sss_detect_ml_helper(
	...
	double log_lik = -sum(elem_div(elem_mult(real(diff), real(diff)), sss_h12_np_est)) -
					 sum(elem_div(elem_mult(imag(diff), imag(diff)), sss_h12_np_est));
...
Cell sss_detect(
	...
	sss_detect_getce_sss(cell, capbuf, fc_requested, fc_programmed, fs_programmed, sss_h1_np_est, sss_h2_np_est,
						 sss_h1_nrm_est, sss_h2_nrm_est, sss_h1_ext_est, sss_h2_ext_est, sampling_carrier_twist,
						 tdd_flag);
	// Perform maximum likelihood detection
	sss_detect_ml(cell, sss_h1_np_est, sss_h2_np_est, sss_h1_nrm_est, sss_h2_nrm_est, sss_h1_ext_est, sss_h2_ext_est,
				  log_lik_nrm, log_lik_ext);
...
//    k_factor=(fc_requested-cell.freq)/fc_programmed;
	...
	if (tdd_flag == 1) {
		if (cp_type_flag == 0)
			frame_start = cell.ind +
						  (-(2 * (128 + 9) + 1) - 1920 - 2) * 16 / FS_LTE * fs_programmed * k_factor;// TDD NORMAL CP
		else
			frame_start = cell.ind +
						  (-(2 * (128 + 32)) - 1920 - 2) * 16 / FS_LTE * fs_programmed * k_factor; //TDD EXTENDED CP
	} else
...
//    k_factor=(fc_requested-cell_in.freq)/fc_programmed;
	...
	int k_factor_idx;
	if (tdd_flag == 1) {
		cell_in.freq = refine_fo(capbuf, cell_in.cp_type, cell_in.n_id_2, cell_in.freq, fs_programmed,
								 cell_in.frame_start, k_factor_vec, k_factor_idx);
	...
	if (cell_in.cp_type == cp_type_t::NORMAL) {
		if (tdd_flag == 0) {
			...
			first_sss_dft_location =
					cell_in.frame_start + (960 - 128 - 9 - 128) * 16 / FS_LTE * fs_programmed * k_factor; //FDD
		} else {
		...
		if (tdd_flag == 0) {
			...
			first_sss_dft_location =
					cell_in.frame_start + (960 - 128 - 32 - 128) * 16 / FS_LTE * fs_programmed * k_factor; //FDD
		} else {
	...
	vec sss_dft_loc_set = itpp_ext::matlab_range(first_sss_dft_location, 9600 * 16 / FS_LTE * fs_programmed * k_factor,
												 (double) (length(capbuf) - 127 - pss_sss_dist - 100));
  ...
  pss_np=NAN;
	...
	for (uint16 k = 0; k < n_sss; k++) {
		...
		h_raw_fo_pss.set_row(k,
							 extract_psss(capbuf.mid(pss_dft_location, 128), -cell_in.freq, k_factor, fs_programmed));
		...
		sss_raw_fo.set_row(k, extract_psss(capbuf.mid(sss_dft_location, 128), -cell_in.freq, k_factor, fs_programmed) *
							  exp(J * pi * -cell_in.freq / (FS_LTE / 16 / 2) * -pss_sss_dist));
		sss_raw_fo.set_row(k, elem_mult(sss_raw_fo.get_row(k),
										to_cvec(ROM_TABLES.sss_fd(cell_in.n_id_1, cell_in.n_id_2, sn))));
...
Cell tfoec(
	...
	for (uint8 sym_num = 0; sym_num <= n_symb_dl - 3; sym_num += n_symb_dl - 3) {
		...
		for (uint16 t = 0; t < n_slot; t++) {
			// Extract RS
			rs_extracted.set_row(t, tfg.get_row(t * n_symb_dl + sym_num).get(
					itpp_ext::matlab_range((uint32) rs_dl.get_shift(mod(t, 20), sym_num, 0), (uint32) 6, (uint32) 71)));
			// Compensate
...
void ce_interp_hex_extend(
	...
	if (itpp_ext::last(row_x) != 71) {
		...
		row_val.ins(len, row_val(len - 1) + (71 - itpp_ext::last(row_x)) * (row_val(len - 1) - row_val(len - 2)) /
											(row_x(len - 1) - row_x(len - 2)));
		row_x.ins(len, 71);
	}
}
...
void chan_est(
	...
	if (port <= 1) {
		...
		Sort<int> sort;
		rs_set = concat(itpp_ext::matlab_range(0, n_symb_dl, n_ofdm - 1),
						itpp_ext::matlab_range(n_symb_dl - 3, n_symb_dl, n_ofdm - 1));
	...
	for (uint16 t = 0; t < n_rs_ofdm; t++) {
		...
		// Extract
		cvec raw_row = tfg.get_row(rs_set(t)).get(
				itpp_ext::matlab_range((int32) rs_dl.get_shift(mod(slot_num, 20), sym_num, port), 6, 71));
...
Cell decode_mib(
	...
	for (uint8 frame_timing_guess = 0; frame_timing_guess <= 3; frame_timing_guess++) {
		...
		ivec ofdm_sym_set = itpp_ext::matlab_range(ofdm_sym_set_start,
												   ofdm_sym_set_start + 3 * 10 * 2 * n_symb_dl + 2 * n_symb_dl - 1);
        ...
        np=NAN;
			...
			if (crc_est == c_est(24, -1)) {
				...
				// Calculate SFN
				int8 sfn_temp = 128 * c_est_ivec(6) + 64 * c_est_ivec(7) + 32 * c_est_ivec(8) + 16 * c_est_ivec(9) +
								8 * c_est_ivec(10) + 4 * c_est_ivec(11) + 2 * c_est_ivec(12) + c_est_ivec(13);
