...
/*! \brief Struct used to keep track of which cell and which port has been displayed on a particular line. */
typedef struct {
	bool occupied;
...
/**
 * \brief Display status information for a certain cell on a certain line on the screen.
 */
void display_cell(
	...
	if (tracked_cell.duplex_mode == 0) {
		printw("[FDD Cell ID %3i][TO: %7.1lf]", tracked_cell.n_id_cell, tracked_cell.frame_timing());
	} else {
		printw("[TDD Cell ID %3i][TO: %7.1lf]", tracked_cell.n_id_cell, tracked_cell.frame_timing());
	}
	...
	attron(COLOR_PAIR(BLUE));
	if (fifo_status) {
		printw("[buffer %5i/%5i]", tracked_cell.fifo.size(), tracked_cell.fifo_peak_size);
	}
	printw("\n");
	attroff(COLOR_PAIR(BLUE));
...
void display_thread(
	...
	if (LINES < LINES_MIN) {
		//endwin();
		cerr << "Error: resize window so it has at least " << LINES_MIN << " rows and " << COLS_MIN << " columns"
			 << endl;
		ABORT(-1);
	}
	if (COLS < COLS_MIN) {
...
#define N_DETAILS 2
	...
	while (true) {
		...
		if (disp_mode == STD) {
			...
			{
				stringstream ss;
				ss << "LTE-Tracker www.evrytania.com; 1.0 to " << MAJOR_VERSION << "." << MINOR_VERSION << "."
				   << PATCH_LEVEL << ": An enhanced LTE Cell Scanner/tracker. Xianjun Jiao (putaoshu@msn.com).";
				move(0, 0);
			...
			if (global_thread_data.cell_seconds_dropped() || global_thread_data.raw_seconds_dropped()) {
				...
				printw("[dropped cell/raw data: %i/%i s]\n", global_thread_data.cell_seconds_dropped(),
					   global_thread_data.raw_seconds_dropped());
			...
			printw("[InitFO: %5.0lfHz][TrackFO: %5.2lfHz]", global_thread_data.initial_frequency_offset(),
				   global_thread_data.frequency_offset() - global_thread_data.initial_frequency_offset());
			//attron(A_BOLD);
//      printw("[searcher delay: %.1lf s]",global_thread_data.searcher_cycle_time());
			printw("[SearcherDelay: %4.2fs][rqst %5.2lfMHz][prog %5.2lfMHz][fs %5.2lfMHz][k %5.3lf][twist %d]",
				   global_thread_data.searcher_cycle_time(), global_thread_data.fc_requested / 1e6,
				   global_thread_data.fc_programmed / 1e6, global_thread_data.fs_programmed / 1e6,
				   global_thread_data.k_factor(), global_thread_data.sampling_carrier_twist());
			//attroff(A_BOLD);
			...
			*/
...

			{
				...
				while (it != tracked_cell_list.tracked_cells.end()) {
					tracked_cell_t &tracked_cell = (*(*it));
					if ((tracked_cell.n_id_cell == n_id_cell) &&
						((port_num == -1) || (port_num < tracked_cell.n_ports))) {
						cell_found = true;
						break;
					}
					++it;
