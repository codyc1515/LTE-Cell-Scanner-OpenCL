...
// Simple usage screen.
void print_usage() {
	cout << "LTE-Tracker (" << BUILD_TYPE << ") help. 1.0 to " << MAJOR_VERSION << "." << MINOR_VERSION << "."
		 << PATCH_LEVEL << ": An enhanced LTE Cell Scanner/tracker. Xianjun Jiao (putaoshu@msn.com)" << endl << endl;
	...
	cout << "    -t --twisted" << endl;
	cout
			<< "      disable sampling-carrier-twisted mode to support external mixer/LNB (default is enable and using carrier&sampling twisted fast search )"
			<< endl;
	cout << "    -p --ppm ppm" << endl;
...
//  }
	...
	if (verbosity >= 1) {
		cout << "OpenCL LTE Tracker (" << BUILD_TYPE << ") beginning. 1.0 to " << MAJOR_VERSION << "." << MINOR_VERSION
			 << "." << PATCH_LEVEL << ": An enhanced LTE Cell Scanner/tracker. Xianjun Jiao (putaoshu@msn.com)" << endl;
...
void dedup(const list<Cell> &detected_cells, list<Cell> &cells_final) {
	cells_final.clear();
...
string freq_formatter(const double &freq) {
	stringstream temp;
	if (abs(freq) < 998.0) {
...
//    ABORT(-1);
	...
	if ((device_index < 0) || (device_index >= n_rtlsdr)) {
		...
		for (uint8 t = 0; t < n_rtlsdr; t++) {
			...
			cerr << "Device index " << t << ": [Vendor: " << vendor << "] [Product: " << product << "] [Serial#: "
				 << serial << "]" << endl;
		}
//    ABORT(-1);
		return (1);
...
double kalibrate(
	...
	if (dongle_used && fc_requested != 9999e6) {
...
#ifdef HAVE_RTLSDR
			fc_programmed_tmp = calculate_fc_programmed_in_context(fc_requested, use_recorded_data, load_bin_filename,
																   rtlsdr_dev);
#endif
		...
		cout << "Use dongle begin with " << (fc_requested / 1e6) << "MHz actual " << (fc_programmed_tmp / 1e6) << "MHz "
			 << fs_programmed << "MHz\n";
	} else {
		...
		cout << "Use file begin with " << (fc_requested_tmp / 1e6) << "MHz actual " << (fc_programmed_tmp / 1e6)
			 << "MHz " << fs_programmed << "MHz\n";
	}
	...
	cout << "    Search frequency: " << fc_search_set(0) / 1e6 << " to "
		 << fc_search_set(length(fc_search_set) - 1) / 1e6 << " MHz" << endl;
	...
	lte_ocl.setup_filter_mchn((string) "filter_mchn_simple_kernel.cl", CAPLENGTH, length(f_search_set) * 3,
							  pss_fo_set.cols(), xcorr_workitem);
#else
...
//    }
		int run_out_of_data = capture_data(fc_requested, correction, false, record_bin_filename, use_recorded_data,
										   load_bin_filename, ".", rtlsdr_dev, hackrf_dev, bladerf_dev, dev_use, capbuf,
										   fc_programmed, fs_programmed, false);
		if (run_out_of_data) {
...
//    cout << "6RB filter cost " << tt.get_time() << "s\n";
		...
		sampling_ppm_f_search_set_by_pss(lte_ocl, 0, capbuf, pss_fo_set, sampling_carrier_twist, num_reserve,
		...
		if (!sampling_carrier_twist) {
			...
			for (uint16 i = 0; i < length(k_factor_set); i++) {
				...
				xcorr_pss(capbuf, tmp_f_search, DS_COMB_ARM, fc_requested, fc_programmed, fs_programmed, tmp_xc,
						  ...
						  sp_incoherent, sp, n_comb_xc, n_comb_sp, sampling_carrier_twist,
						  (const double) k_factor_set[i]);
			...
			peak_search(xc_incoherent_collapsed_pow, xc_incoherent_collapsed_frq, Z_th1, dynamic_f_search_set,
						fc_requested, fc_programmed, xc_incoherent_single, DS_COMB_ARM, sampling_carrier_twist, NAN,
						peak_search_cells);
...
int main(
	...
	parse_commandline(argc, argv, fc_requested, ppm, correction, device_index, expert_mode, use_recorded_data, filename,
					  repeat, drop_secs, rtl_sdr_format, noise_power, initial_sampling_carrier_twist,
					  record_bin_filename, load_bin_filename, opencl_platform, opencl_device, filter_workitem,
					  xcorr_workitem, num_reserve, gain);
...
#ifdef HAVE_RTLSDR
		if (config_rtlsdr(initial_sampling_carrier_twist, correction, device_index, fc_requested, rtlsdr_dev,
						  fs_programmed, gain) == 0) {
...
//  cout << rtlsdr_dev << " " << hackrf_dev << " " << bladerf_dev << " " <<  dev_use << " " <<  fc_programmed << " " <<  initial_sampling_carrier_twist << " " <<  initial_k_factor << " " <<  record_bin_filename << " " <<  load_bin_filename << " " <<  opencl_platform << " " <<  opencl_device << " " <<  filter_workitem << " " <<  xcorr_workitem << " " <<  num_reserve << "\n";
	double initial_freq_offset = kalibrate(fc_requested, fs_programmed, ppm, correction, correction_new,
										   use_recorded_data, filename, rtl_sdr_format, noise_power, drop_secs, repeat,
										   rtlsdr_dev, hackrf_dev, bladerf_dev, dev_use, fc_programmed,
										   initial_sampling_carrier_twist, initial_k_factor, record_bin_filename,
										   load_bin_filename, opencl_platform, opencl_device, filter_workitem,
...
//  sampbuf_sync_t sampbuf_sync;
	...
	*/
	...
	boost::thread searcher_thr(searcher_thread, boost::ref(capbuf_sync), boost::ref(global_thread_data),
							   boost::ref(tracked_cell_list));

	// Start the producer thread.
	boost::thread producer_thr(producer_thread, boost::ref(sampbuf_sync), boost::ref(capbuf_sync),
							   boost::ref(global_thread_data), boost::ref(tracked_cell_list),
							   boost::ref(fc_programmed));

	sampbuf_sync.fifo_peak_size = 0;

	// Launch the display thread
	boost::thread display_thr(display_thread, boost::ref(sampbuf_sync), boost::ref(global_thread_data),
							  boost::ref(tracked_cell_list), boost::ref(expert_mode));
...
//    //cout << db10(sigpower(file_data)) << endl;
		capture_data(fc_requested, correction, false, record_bin_filename, use_recorded_data, load_bin_filename, ".",
					 rtlsdr_dev, hackrf_dev, bladerf_dev, dev_use, file_data, fc_programmed, fs_programmed, true);
